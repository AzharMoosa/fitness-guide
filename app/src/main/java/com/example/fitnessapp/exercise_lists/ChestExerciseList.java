...
public class ChestExerciseList extends AppCompatActivity {

  private LinearLayout chestExerciseList;

  @Override
  protected void onCreate(Bundle savedInstanceState) {
    super.onCreate(savedInstanceState);
    setContentView(R.layout.activity_chest_exercise_list);
    chestExerciseList = findViewById(R.id.chest_exercises_list);
    getChestExercises();
  }

  private void getChestExercises() {
    ApiUtilities.getApiInterface()
        .getExercisesByType("Chest Exercises").enqueue(new Callback<List<ExerciseData>>() {
      @Override
      public void onResponse(Call<List<ExerciseData>> call,
        ...
        if (response.body() != null) {
          ...
          for (ExerciseData exercise : exercises) {
            chestExerciseList.addView(createRow(exercise));
          }
        }
      }
      ...
      public void onFailure(Call<List<ExerciseData>> call, Throwable t) {
        Toast.makeText(getApplicationContext(), "Cannot Get Chest Exercises", Toast.LENGTH_SHORT).show();
      }
    });
  }
